...
import { Section, SectionCard } from "./Setting";
...
export function CommandLineSection() {
	...
	const handleInstall = async () => {
		...
		try {
			...
			toast.success("Shelf CLI installed");
		} catch (error) {
	...
	const handleUninstall = async () => {
		...
		try {
			...
			toast.success("Shelf CLI removed");
		} catch (error) {
	...
	return (
		<Section
			title="Command Line"
			description="Install the Shelf command for terminals, agents, scripts, and local automation."
		>
			<SectionCard padded>
				<Show
					...
					fallback={
						<Show
							when={status.error}
							fallback={<div class="h-20 rounded-lg bg-gray-3 animate-pulse" />}
						>
					...
					{(currentStatus) => (
						<div class="flex flex-col gap-4">
							<div class="flex items-start justify-between gap-4">
								<div class="flex flex-col gap-1 min-w-0">
									<p class="text-[13px] text-gray-12">
										{currentStatus().installed ? "Installed" : "Not installed"}
									</p>
							...
							<div class="grid gap-2 text-xs">
								...
								<PathRow label="Target" value={currentStatus().targetPath} />
							</div>
							...
							<Show when={currentStatus().installed && !currentStatus().onPath}>
										...
										<Show
											...
											fallback={
												<>
													...
													to your PATH to use{" "}
													<code class="font-mono text-gray-12">shelf</code> from
													a new terminal.
												</>
											}
										>
											Added <code class="font-mono text-gray-12">shelf</code> to
											your PATH. Restart your terminal to use it, or run this
											now:
										</Show>
									</p>
