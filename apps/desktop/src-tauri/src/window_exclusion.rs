...
mod tests {
    ...
    fn matches_by_window_title() {
        let exclusion = title_exclusion("Shelf Camera");
        assert!(exclusion.matches(None, None, Some("Shelf Camera")));
    ...
    fn matches_by_owner_name() {
        let exclusion = owner_exclusion("Shelf");
        assert!(exclusion.matches(None, Some("Shelf"), None));
    ...
    fn matches_owner_and_title_requires_both() {
        let exclusion = WindowExclusion {
            bundle_identifier: None,
            owner_name: Some("Shelf".to_string()),
            window_title: Some("Shelf Camera".to_string()),
        };
        assert!(exclusion.matches(None, Some("Shelf"), Some("Shelf Camera")));
        assert!(!exclusion.matches(None, Some("Shelf"), Some("Wrong Title")));
        assert!(!exclusion.matches(None, Some("Wrong Owner"), Some("Shelf Camera")));
        assert!(!exclusion.matches(None, None, Some("Shelf Camera")));
    }
    ...
    fn instant_mode_removes_camera_exclusion() {
        let exclusions = vec![
            ...
            title_exclusion("Shelf Camera"),
            title_exclusion("Shelf Settings"),
            title_exclusion("Shelf Recording Controls"),
        ];

        let filtered = filter_for_instant_mode(exclusions, "Shelf Camera");

        assert_eq!(filtered.len(), 3);
        assert!(
            filtered
                .iter()
                .all(|e| e.window_title.as_deref() != Some("Shelf Camera"))
        );
        assert!(
            filtered
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Shelf"))
        );
        assert!(
            filtered
                .iter()
    ...
    fn instant_mode_handles_empty_list() {
        let filtered = filter_for_instant_mode(vec![], "Shelf Camera");
    ...
    fn default_exclusions_contain_camera() {
        ...
        assert!(
            defaults
                .iter()
                .any(|e| e.window_title.as_deref() == Some("Shelf Camera")),
            "Default exclusions must include 'Shelf Camera' — instant mode filtering depends on this"
        );
    }
    ...
    fn default_exclusions_only_match_camera_by_title() {
        ...
        let camera = defaults
            .iter()
            .find(|e| e.window_title.as_deref() == Some("Shelf Camera"))
            .expect("Shelf Camera must be in default exclusions");
