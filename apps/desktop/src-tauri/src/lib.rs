...
fn build_macos_app_menu(app_handle: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let quit_label = config
        ...
        .unwrap_or_else(|| "Quit Shelf".to_string());
...
fn configure_windows_graphics_recovery(
    ...
    if should_engage_graphics_recovery(previous_termination) {
        ...
        warn!(
            "Previous Shelf session terminated during GPU initialisation; using Windows software graphics recovery mode for this launch"
        );
...
fn configure_camera_blur_recovery(
    ...
    if next.is_some() {
        ...
        if stored.is_none() {
            error!(
                "Previous Shelf session died with camera background blur active; disabling blur until the next app update"
            );
        } else {
    ...
    builder
        ...
        .setup(move |app| {
            ...
            tokio::spawn({
                let app = app.clone();
                async move {
                    mark_crashed_recordings_failed(app)
                        .await
                        .map_err(|err| warn!("Error marking crashed recordings: {err}"))
                        .ok();
                }
            });
