...
/// Captures a whole display so an area selected later can be cut out of it.
///
/// See [`crop_frozen_display`] for why this has to happen before the selection
/// overlay is on screen.
#[cfg(not(target_os = "linux"))]
pub async fn capture_frozen_display(id: scap_targets::DisplayId) -> anyhow::Result<RgbImage> {
    let image = capture_screenshot(ScreenCaptureTarget::Display { id }).await?;
    Ok(image.into_rgb8())
}

#[cfg(target_os = "linux")]
pub async fn capture_frozen_display(_id: scap_targets::DisplayId) -> anyhow::Result<RgbImage> {
    Err(anyhow!("Freezing a display is not implemented on Linux"))
}

/// Cuts an area out of a display captured earlier by [`capture_frozen_display`].
///
/// An area screenshot normally captures at the moment the selection is
/// confirmed, and by then anything that reacted to the pointer is gone: opening
/// the overlay takes the mouse away from the window underneath, so a hovered
/// button has fallen back to its resting state and a tooltip has disappeared
/// entirely. Freezing the display while the pointer is still where the user left
/// it, then cutting the selection out of that image, keeps what was really on
/// screen when the shortcut was pressed.
///
/// The frozen image covers the whole display in physical pixels, which is
/// exactly what the normal area path produces before it crops, so the same crop
/// runs here.
#[cfg(not(target_os = "linux"))]
pub fn crop_frozen_display(
    image: RgbImage,
    target: &ScreenCaptureTarget,
) -> anyhow::Result<DynamicImage> {
    let ScreenCaptureTarget::Area { bounds, screen } = target else {
        return Err(anyhow!("A frozen display can only be cropped to an area"));
    };
    ...
    let logical = display
        .logical_size()
        .ok_or_else(|| anyhow!("Display reported no logical size"))?;

    let rect = frozen_crop_rect(image.width(), image.height(), logical, *bounds)
        .ok_or_else(|| anyhow!("Selection lies outside the frozen display"))?;
...
/// Works out which pixels of a frozen display the selection covers.
///
/// The scale is derived from the captured image rather than read from the
/// display, because the two disagree whenever a Mac runs a scaled resolution:
/// macOS renders at twice the logical size and then resamples to the panel, so
/// a display reporting a backing scale of 2 can hand back an image that is not
/// twice its logical width. Dividing the image by the logical size is right in
/// every mode, including the ordinary unscaled one.
///
/// Returns `None` when the selection and the display do not overlap at all;
/// a selection running off an edge is clamped to what the display holds.
#[cfg(not(target_os = "linux"))]
fn frozen_crop_rect(
    image_width: u32,
    image_height: u32,
    display_logical: scap_targets::bounds::LogicalSize,
    bounds: scap_targets::bounds::LogicalBounds,
) -> Option<CropRect> {
    if display_logical.width() <= 0.0 || display_logical.height() <= 0.0 {
        return None;
    }

    let scale_x = f64::from(image_width) / display_logical.width();
    let scale_y = f64::from(image_height) / display_logical.height();

    let left = (bounds.position().x() * scale_x).round().max(0.0) as u32;
    let top = (bounds.position().y() * scale_y).round().max(0.0) as u32;
    let width = (bounds.size().width() * scale_x).round().max(0.0) as u32;
    let height = (bounds.size().height() * scale_y).round().max(0.0) as u32;

    let x = left.min(image_width);
    let y = top.min(image_height);
    let width = width.min(image_width.saturating_sub(x));
    let height = height.min(image_height.saturating_sub(y));

    (width > 0 && height > 0).then_some(CropRect {
        x,
        y,
        width,
        height,
...
mod frozen_crop_tests {
    use super::frozen_crop_rect;
    ...
    #[test]
    fn a_retina_display_doubles_every_coordinate() {
        let rect = frozen_crop_rect(
            3456,
            2234,
            LogicalSize::new(1728.0, 1117.0),
            bounds(100.0, 50.0, 400.0, 300.0),
        )
        .expect("selection lies on the display");
    ...
    #[test]
    fn an_unscaled_display_maps_one_to_one() {
        let rect = frozen_crop_rect(
            1920,
            1080,
    ...
    /// The case the display's own scale factor gets wrong.
    ///
    /// A 14" MacBook Pro set to "More Space" has a logical size of 1800x1169 on
    /// a 3024x1964 panel, so the real ratio is 1.68 while the display still
    /// reports a backing scale of 2. Multiplying by the reported scale would put
    /// this selection at x=1800 instead of x=1512, nearly 300 pixels off.
    #[test]
    fn a_scaled_resolution_follows_the_image_not_the_reported_scale() {
        let rect = frozen_crop_rect(
            3024,
            1964,
            LogicalSize::new(1800.0, 1169.0),
            bounds(900.0, 584.5, 450.0, 292.25),
        )
        .expect("selection lies on the display");
        ...
        assert_eq!(
            (rect.x, rect.y),
            (1512, 982),
            "a reported scale of 2 would have given (1800, 1169)"
        );
        assert_eq!((rect.width, rect.height), (756, 491));
    }

    #[test]
    fn a_selection_running_off_the_edge_is_clamped() {
        let rect = frozen_crop_rect(
            1000,
            1000,
            LogicalSize::new(1000.0, 1000.0),
            bounds(900.0, 900.0, 500.0, 500.0),
        )
        .expect("part of the selection lies on the display");

        assert_eq!((rect.x, rect.y), (900, 900));
        assert_eq!((rect.width, rect.height), (100, 100));
    }

    #[test]
    fn a_selection_beyond_the_display_is_refused() {
        assert!(
            frozen_crop_rect(
                1000,
                1000,
                LogicalSize::new(1000.0, 1000.0),
                bounds(1000.0, 1000.0, 200.0, 200.0),
            )
            .is_none()
        );
    }
    ...
    #[test]
    fn a_display_reporting_no_size_is_refused() {
        assert!(
            frozen_crop_rect(
                1000,
                1000,
