...
/// Cap fork: the window store is a `RefCell` behind an `unsafe impl Sync`. In
/// practice its borrow flag can end up stuck in the "mutably borrowed" state
/// even though nothing holds a mutable borrow, after which every `borrow()`
/// panics on the main thread and kills the app (tauri-apps/tauri#14801,
/// tauri-apps/tauri#15003). Cap hits this when the recordings overlay opens.
///
/// Every mutable borrow in this crate goes through [`win_borrow_mut`], which
/// counts live writers. A read that cannot take the borrow therefore knows
/// whether a real writer exists: if none does, the flag is stuck and the map is
/// read directly; if one does, the read is skipped instead of racing with it.
static WINDOW_STORE_WRITERS: std::sync::atomic::AtomicUsize =
  std::sync::atomic::AtomicUsize::new(0);

fn empty_window_store() -> &'static WindowsStore {
  static EMPTY: std::sync::OnceLock<WindowsStore> = std::sync::OnceLock::new();
...
impl<'a> std::ops::Deref for WindowsRead<'a> {
...
struct WindowStoreWriterCount;

impl WindowStoreWriterCount {
  fn new() -> Self {
    WINDOW_STORE_WRITERS.fetch_add(1, std::sync::atomic::Ordering::Release);
    Self
  }
}

impl Drop for WindowStoreWriterCount {
  fn drop(&mut self) {
    WINDOW_STORE_WRITERS.fetch_sub(1, std::sync::atomic::Ordering::Release);
  }
}
...
impl<T: UserEvent> Runtime<T> for Wry<T> {
  ...
  fn create_webview(
    ...
    let window = crate::win_borrow(&self.context.main_thread.windows.0, "create_webview")
      .get(&window_id)
...
fn handle_user_message<T: UserEvent>(
  ...
  match message {
    ...
    Message::Window(id, window_message) => {
      let w = crate::win_borrow(&windows.0, "window message").get(&id).map(|w| {
        (
          w.inner.clone(),
          w.webviews.clone(),
    ...
    Message::Webview(window_id, webview_id, webview_message) => {
      ...
      let webview_handle = crate::win_borrow(&windows.0, "webview message").get(&window_id).map(|w| {
        (
          w.inner.clone(),
    ...
    Message::CreateWebview(window_id, handler) => {
      let window = crate::win_borrow(&windows.0, "CreateWebview")
        .get(&window_id)
        .map(|w| (w.inner.clone(), w.focused_webview.clone()));
      if let Some((Some(window), focused_webview)) = window {
...
fn handle_event_loop<T: UserEvent>(
  ...
  match event {
    ...
    Event::UserEvent(Message::Webview(
      ...
      let windows_ref = crate::win_borrow(&windows.0, "window event");
    ...
    Event::UserEvent(Message::Webview(
      ...
      if let Some(event) = WindowEventWrapper::from(event).0 {
        let windows_ref = crate::win_borrow(&windows.0, "window focus event");
    ...
    Event::WindowEvent {
      ...
      if let Some(window_id) = window_id_map.get(&window_id) {
        {
          let windows_ref = crate::win_borrow(&windows.0, "window theme event");
          if let Some(window) = windows_ref.get(&window_id) {
        ...
        match event {
          ...
          TaoWindowEvent::ThemeChanged(theme) => {
            if let Some(window) = crate::win_borrow(&windows.0, "window scale event").get(&window_id) {
          ...
          TaoWindowEvent::Resized(size) => {
            if let Some((Some(window), webviews)) = crate::win_borrow(&windows.0, "Resized")
              .get(&window_id)
...
fn on_close_requested<'a, T: UserEvent>(
  ...
  let (tx, rx) = channel();
  let windows_ref = crate::win_borrow(&windows.0, "close requested");
